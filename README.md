...
프로그래밍에서 필요한 데이터를 추상화시켜 <br>
상태와 행위를 가진 객체를 만들고 <br>
...
<details>
<summary> 객체지향의 장점, 단점에 대해서 설명해주세요. </summary>
<div markdown="1">  
<br>
...
- 유지보수가 쉬움<br>
  - 절차 지향 프로그래밍에서는 코드를 수정해야할 때 일일이 찾아 수정해야하는 반면<br>
  - 객체 지향 프로그래밍에서는 수정해야 할 부분이 클래스 내부에 멤버 변수혹은 메서드로 존재하기 때문에 해당 부분만 수정하면 된다. <br><br>

- 대형 프로젝트에 적합<br>
  - 클래스 단위로 모듈화시켜서 개발할 수 있으므로 대형 프로젝트처럼 여러 명, 여러 회사에서 프로젝트를 개발할 때 업무 분담하기 쉽다.<br>

<br><br>

[ 단점 ] <br>
- 설계시 많은 시간과 노력이 필요<br>
- 객체가 많으면 용량이 커질 수 있음<br>
...
- 각각의 클래스마다 속성으로 model, color, wheels 기능으로 moveForward()와 moveBackward 가 완전히 동일한 코드임에도 불구하고 계속 반복될 수 있습니다.  <br>
- 또한 하나의 코드에서 변경 사항이 일어나면, 해당 코드의 변경 사항을 다른 클래스에서도 일일이 수정해주어야 하는 어려움이 있습니다. <br>

<br><br>


그러면 이제 앞서 설명했던 추상화와 상속을 활용하여 앞선 코드를 재정의해보도록 하겠습니다. <br> <br> 
...
## 3. 다형성 <br>
  ...
  - 대표적인 앞서 본 메서드 오버라이딩과 메서드 오버로딩(method overloading)이 있음  <br><br>

[ Vehicle 인터페이스 ] <br>

![image](https://github.com/developerOlive/Backend_interview_question/assets/67456294/c03f38ab-7fd0-49ab-acef-2e88c7b72910)

<br>

[ Car 클래스 ]  <br>

![image](https://github.com/developerOlive/Backend_interview_question/assets/67456294/196f774d-ebe8-4974-b8da-2fbb4bb42857)

<br>

- 메서드 오버라이딩을 사용하면 같은 이름의 moveForward() 와 moveBackward() 를 각각의 클래스의 맥락에 맞게 재정의하여 사용할 수 있습니다.<br>
- 즉, 같은 이름의 메서드가 상황에 따라 다른 역할을 수행하는 것입니다.<br>
- 또한, 하나의 클래스 내에서 같은 이름의 메서드를 여러 개 중복하여 정의하는 것을 의미하는 메서드 오버로딩도 이와 같은 맥락이라 할 수 있습니다.<br><br>


사실 앞서 언급한 메서드 오버라이딩과 메서드 오버로딩도 다형성의 한 중요한 예시지만, 객체 지향의 맥락에서 이것보다 더 중요한 다형성의 정의는 이것입니다.<br><br>

객체 지향 프로그래밍에서 다형성이란 한 타입의 참조변수를 통해 여러 타입의 객체를 참조할 수 있도록 만든 것을 의미합니다. <br>
좀 더 구체적으로, 상위 클래스 타입의 참조변수로 하위 클래스의 객체를 참조할 수 있도록 하는 것입니다.<br><br>

지금까지 쭉 사용해왔던 이동 수단 을 다시 한번 더 예시로 들어보면, 이동수단 은 자동차 가 될 수도, 오토바이 가 될 수도 있습니다. <br>
다르게 표현해보면, '자동차는 자동차이다.' 라는 명제와 '자동차는 이동수단이다.' 라는 명제는 모두 참입니다. <br>
오토바이의 경우도 마찬가지 일 것입니다. <br>
이동수단이라는 범위 안에 자동차와 오토바이를 하나로 묶을 수 있게 됩니다. <br>
즉, 이동수단은 작은 개념들을 품을 수 있는 포괄적인 개념이라는 의미입니다.<br><br>

다시 위의 정의로 돌아가 보면, 객체 지향 프로그래밍에서 다형성이란 
앞서 설명한 이동 수단 과 같은 넓은 범위의 타입, 즉 상위 클래스 타입의 참조 변수로 
그것과 관계있는 하위 클래스들을 참조할 수 있는 능력입니다. <br>

이제 코드를 통해 좀 더 구체적으로 그 의미를 알아보도록 하겠습니다.<br><br>

[ Main 실행 클래스 ] <br>

![image](https://github.com/developerOlive/Backend_interview_question/assets/67456294/e3fd60f9-9005-4335-acec-b0f489809758)

<br><br>
이제 방금 설명한 다형성 개념을 활용하여 객체를 생성하는 방법을 추가해보도록 하겠습니다.<br>

[ Main 실행 클래스 ] <br>

![image](https://github.com/developerOlive/Backend_interview_question/assets/67456294/582ed6bd-d9d1-46e6-ad04-55267bc71bbf)


위의 코드를 확인해보면, 상위클래스 타입의 참조변수로 하위클래스 객체를 참조하는 것 의 의미를 조금 더 구체적으로 이해할 수 있습니다. <br>
원래 우리가 사용했던 방식은 하위 클래스의 객체를 생성하여 하위 클래스 타입의 참조 변수에 할당해주었지만, <br>
다형성을 활용한 객체 생성 방식에서는 하위 클래스의 객체를 생성하여 상위 클래스 타입의 참조변수 car2 에 할당해주고 있습니다.<br><br>
...
- DI가 필요한 이유(DI의 장점) <br>
  - 강하게 결합된 클래스들을 분리하고, 애플리케이션 실행 시점에 객체 간의 관계를 결정해 줌으로써 결합도를 낮추고 유연성을 확보해준다. <br>
  - 이러한 방법은 상속보다 훨씬 유연하다. <br>
    - 두 객체 간의 관계라는 관심사의 분리 <br>
    - 두 객체 간의 결합도를 낮춤 <br>
    - 객체의 유연성을 높임 <br>
    - 테스트 작성을 용이하게 함 <br>
